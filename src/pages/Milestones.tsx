          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-12">
              ...
              <div className="overflow-x-auto pb-4">
                <ol role="list" className="relative border-t border-blue-100 mt-2 flex flex-row min-w-max gap-8 px-4">
                  {milestones.map((m) => (
                    <li key={m.id} className="pt-6 relative w-48 shrink-0">
                      <button
                        onClick={() => setSelected(m.id)}
                        aria-current={selected === m.id ? "true" : undefined}
                        aria-label={`View details for ${m.label}`}
                        className={`absolute top-[-11px] left-1/2 -translate-x-1/2 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                          selected === m.id
                      ...
                      <button
                        onClick={() => setSelected(m.id)}
                        className={`text-center transition-all block w-full mt-2`}
                        aria-label={`Select ${m.label}`}
                      >
                        <div className="flex flex-col items-center gap-1 mb-1">
                          <span
                            className={`text-[0.9rem] font-semibold transition-colors ${
                              selected === m.id ? "text-blue-700" : "text-slate-800 hover:text-blue-700"
                            }`}
                          >
                            {m.label}
                          </span>
